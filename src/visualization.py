...
class Visualization:
    def __init__(self, myMap, nRegions):
        ...
        ctk.CTkLabel(master=intro, text='1. The Agent, Pirate, and Treasure denote by "A", "Pi", "T"', font=(
            'Roboto', 20)).place(x=0, y=80)
        ctk.CTkLabel(master=intro, text='2. The red-border cells: hints given by the pirate', font=(
            'Roboto', 20)).place(x=0, y=120)
        ctk.CTkLabel(master=intro, text='3. The gray cells: areas has been removed by the agent', font=(
    ...
    def createNewTab(self, nRound, agent, pirate, treasure, hintTiles, removeTiles, freed, logs):
        ...
        # the map
        map = ctk.CTkCanvas(master=mapDisplay, width=mapSize,
                            height=mapSize, highlightthickness=0)
        ...
        for r in range(self.w):
            for c in range(self.h):
                ...
                cellType = None if self.map[r][c][-1] not in ['M',
                                                              'P'] else self.map[r][c][-1]
                region = int(self.map[r][c]) if cellType is None and 'T' not in self.map[r][c] else int(
                    self.map[r][c][:-1])
        ...
        textBox = ctk.CTkTextbox(
            log, width=500, height=500, font=("Roboto", 15))
