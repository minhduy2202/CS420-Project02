...
def calAvgTiles(_removed: set, _agent: int, _w: int, _h: int):
    ...
    for i in range(_w * _h):
        if i not in _removed:
            ...
            cnt += 1
    if cnt:
        return abs(x / cnt - _agent // _w) + abs(y / cnt - _agent % _w)
    return 1000000000
...
def getBestAction(_agent: int, _w: int, _h: int, _hints: list, _removed: list, _map: list):
    ...
    for i in range(8):
        ...
        if x < 0 or x >= _h or y < 0 or y >= _w or check[i % 4]:
            continue
    ...
    if ac != 0:
        heapq.heappush(actions, (-(remove + min(_w * _h -
                       len(_removed) + 1, 6)), "move and small scan", ac))

    # large move
    moves = [(0, 3), (3, 0), (-3, 0), (0, -3),
             (0, 4), (4, 0), (-4, 0), (0, -4)]
    ac = 0
    for i in range(8):
        move = moves[i]
    ...
    if ac != 0:
        heapq.heappush(
            actions, (-(min(_w * _h - len(_removed) + 2, 8)), "move", ac))
    ...
    for hint in _hints:
        ...
        for tile in hint[0][1]:
            if tile not in _removed:
                cnt += 1
        heapq.heappush(
            actions, (-(max(cnt, _w * _h - nRe - cnt)), "verify", hint))
...
def getActions(_w: int, _h: int, _freed: bool, _canTele: bool, _known: bool, _treasure: int, _agent: int, _pirate: int, _prevMove: int, _removed: set, _hints: list, _map: list, _prev: list):
    if _known and _treasure == _agent:
        ...
        if _canTele:
            return (0, "teleport", _pirate)
        if _pirate == _prevMove:
            return (0, "large scan", _agent)
